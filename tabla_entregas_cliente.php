...
<body>
	...
	<center>
		...
		<h1 class="h1 titulo">ENTREGAS POR CLIENTES</h1>
	...
	<table class=" table table-striped table-border table-hover table-sm table-responsive">
		...
		<thead>
			...
			<tr>				 				
				...
				<th colspan="6"><center> Listado de Entregas por clientes</center></th>
...
<?php
      $pen="";
		...
		$statement = $conexion->prepare('SELECT c.nombre,e.numero_pedido,e.referencia,e.color,e.metros,e.metros_e,e.fecha,e.estado FROM entregas e,clientes c WHERE e.cliente=c.id ');
		$statement->execute();
